...
public class MeController {
    ...
    public ResponseEntity<ApiResponse<SuccessResponse>> deleteAccount(@RequestBody az.fitnest.identity.dto.request.DeactivateAccountRequest request) {
        Long userId = UserContext.getRequiredUserId();
        try {
            userService.deactivateAccount(userId, request);
            return ResponseEntity.ok(ApiResponse.success(
                    SuccessResponse.of(getMessage("success.account.deleted"), "/api/v1/me/delete-account")
            ));
        } catch (Exception e) {
