package az.fitnest.identity.user.adapter.service;
...
import az.fitnest.identity.shared.exception.ResourceNotFoundException;
...
public class UserService {
    ...
    public User createNewUser(String firstName, String lastName, String passwordHash, String mobile) {
        mobile = az.fitnest.identity.shared.util.MobileNumberUtils.normalize(mobile);
    ...
    @org.springframework.cache.annotation.CacheEvict(value = "users", key = "#userId")
    @Transactional
    public User updateLanguage(Long userId, az.fitnest.identity.user.domain.enums.Language language) {
        User user = getUserOrThrow(userId);
